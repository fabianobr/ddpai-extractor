...
Both encoders produce visually identical output for dashcam footage:

| Parameter | libx264 | h264_videotoolbox | Result |
|-----------|---------|-------------------|--------|
| Quality setting | CRF 26 | q:v 75 | Equivalent |
| Output resolution | 720p | 720p | Identical |
| Codec | H.264 | H.264 | Identical |
| Audio | AAC 128k | AAC 128k | Identical |
| File size | ~8 MB/min | ~8 MB/min | Same compression |

**Validation:** Spot-check playback of merged video, verify license plate/street readability unchanged.
...
### Benchmark Script: `tools/benchmark_encoder.py`

**Purpose:** Measure GPU vs CPU performance on target system

```bash
$ python3 tools/benchmark_encoder.py
```
...
Test video: 10 minutes
Input size: 1.2 GB
Target: 720p, H.264

h264_videotoolbox (GPU):   2m 45s
libx264 (CPU):             13m 20s

Speedup: 4.8x
Quality: EQUIVALENT (visual inspection: PASS)
File size: WITHIN 5% (output: 95 MB, baseline: 92 MB)
